 ...
 /**-----------------------------------------------------------------------------------------
  ...
  Fecha Creación: 2009/09/23
  Fecha Modificación: 2009/10/06
  Motivo Modificación:  Cambiaron la definición del RICEF y del WSDL
  Fecha Modificación: 2010/04/29
  Motivo Modificación:  Se cambian los amparos adicionales de vida de MVI a AAV email de Miércoles 2010/4/28 02:53 p.m.

  #param ivaNmExpediente       Número del siniestro sobre el cual se genera la CxP
  #param ivaNmPagoAutorizacion Número de la Orden de Pago asociada al siniestro sobre el cual se genera la CxP
...
-----------------------------------------------------------------------------------------------------------
 ...
 /**-----------------------------------------------------------------------------------------
  Function que Genera la Anulación de CxC SALVAMENTOS a ser llevada del modelo de Siniestros a SAP
  #author Juan Guillermo Henao Montoya
  Fecha Creación: 2009/12/14
  Fecha Modificación:
  Motivo Modificación:
 ...
 /**-----------------------------------------------------------------------------------------
  ...
  #param ivaNmAplicacion       Código de la aplicación según SAP para siniestros es la 79
  #param ovaDsUsuario          Código del usuario de SuraBroker
 ...
 /**-----------------------------------------------------------------------------------------
  Funcion que retorna si una póliza o un riesgo de una póliza está contratado en el exterior.
  Si se envía solo la póliza, retorna que SI tiene riesgo en el exterior si alguno de los riesgos está en el exterior.
  Si se envía la póliza y el riesgo trae si este riesgo está ubicado en el exterior
  #author Juan Guillermo Henao Montoya
  Fecha Creación: 2013/08/16
  Fecha Modificación:
  Motivo Modificación:

  #param ivaNmPoliza             Número de la póliza a consultar
  #param ivaNmCertificado        Número del riesgo de la póliza a consultar
  #param ovaMensajeTecnico       Mensaje tecnico en caso de error
  #param ovaMensajeUsuario       Mensaje para el usuario en caso de error
  */
 ...
 /**-----------------------------------------------------------------------------------------
  ...
  El procedimiento genera la contabilización.
  #author Juan Guillermo Henao Montoya
  Fecha Creación: 2013/10/26
  Fecha Modificación:
  Motivo Modificación:

  #param ivaTipoMovimiento       Tipo de movimiento de acuerdo a la operación a ejecutar D es deducible
  #param ivaExpediente           Número del siniestro a crearle el movimiento
  #param ivaDniBeneficiario      DNI completo Tipo y número de la persona a la que se le va a crear el movimiento
  #param inuValorMovimiento      Valor del movimiento a generar
  #param onuNmPagoAutorizacion   Número de orden de pago o reintegro generada
  #param ovaMensajeTecnico       Mensaje tecnico en caso de error
  #param ovaMensajeUsuario       Mensaje para el usuario en caso de error
  */
...
/*  ltabMapItems                      TAB_SAP_MAPITEM           := TAB_SAP_MAPITEM(OBJ_SAP_MAPITEM());
  ...
  -- 01/12/2024 josebuvi contador para crear objeto de retencion por vida solo 1 vez
  ...
  --20140728 Por historia de usuario 77550 solicitada por Mario Franco, ya no se lleva obligado por Caja la pretención objetivada.
...
--  lnuTotalPtDescuento              SIN_PAGOS_DET.PTDESCUENTO%TYPE;
  ...
  --linaduto - 2025/03/11 - HU756348 -Ajuste retención en la fuente para ID NIT - subramos PCP y deudores
  ...
  CURSOR lcuDatosDocumentoFyC IS
   ...
   GROUP BY CDTIPO_RESERVA,NUMERO_RESERVA, NVL(POIRPF,0), S.CDRETENCION,NVL(POIRC,0), NVL(POIRPF_RETENIDO,0), NVL(POICA,0), /*ROUND(ABS(((IMPORTE*100)/(100+NVL(POIRPF,0))))),*/--Se cambia porque está generando mas de un documento cuando no hay cambio de retenciones
            s.CDTRIBUTARIA/*, s.SNIVA*/,
            EXPEDIENTE;

  -- Cursor base para el llenado con los datos del DOCUMENTO por cada tipo de reserva Matemática o Técnica
  ...
  CURSOR lcuDetalleSumadosDeducible IS
   SELECT s.CDRAMO, s.CDSUBRAMO, s.CDGARANTIA, s.CDSUBGARANTIA, DNI,
          DECODE(SUBSTR(EXPEDIENTE,6,1),3,ROUND(ABS(DEDUCIBLE),2),
                 ROUND(ABS(DEDUCIBLE)) ) DEDUCIBLE, s.NUMERO_RESERVA
   FROM SIN_PAGOS_DET s, DIC_ALIAS_COBERTURAS dac
   WHERE EXPEDIENTE               = ivaNmExpediente
   AND   NUMERO_PAGO_AUTORIZACION = ivaNmPagoAutorizacion
   AND   s.CDRAMO                 = dac.CDRAMO
  ...
  CURSOR lcuDetalleSumadosDescuento IS
   SELECT s.CDRAMO, s.CDSUBRAMO, s.CDGARANTIA, s.CDSUBGARANTIA, s.DNI,
          DECODE(SUBSTR(EXPEDIENTE,6,1),3,ROUND(ABS(s.PTDESCUENTO),2),
                 ROUND(ABS(s.PTDESCUENTO)) ) PTDESCUENTO, s.NUMERO_RESERVA
   FROM SIN_PAGOS_DET s, DIC_ALIAS_COBERTURAS dac
   WHERE EXPEDIENTE               = ivaNmExpediente
   AND   NUMERO_PAGO_AUTORIZACION = ivaNmPagoAutorizacion
   AND   s.CDRAMO                 = dac.CDRAMO
...
/*   AND   ctd.NPOLIZA    (+)       = s.NPOLIZA
  ...
  -- Cursor base para el llenado del detalle OBJ_SAP_DETALLE_CXP_SINI por cada tipo de reserva Matematica o Técnica
...
--   AND   CDCIA                 = '00000'
   ...
   AND   ldaFeOcurrencia BETWEEN FEALTA
                             AND NVL(FEBAJA,SYSDATE);
  -- Se omite gastos del proceso jurídico de acuerdo a solicitud de Jose Libardo Cruz con CA 120293
  --20140728 Por historia de usuario 77550 solicitada por Mario Franco, ya no se lleva obligado por Caja la pretención objetivada.
  ...
  -- Fin de 20140728 Por historia de usuario 77550 solicitada por Mario Franco, ya no se lleva obligado por Caja la pretención objetivada.

  -- Panamá.  Para identificar el tipo de producto de acuerdo a la vigencia Si el siniestro está en la priver vigencia
  ...
  -- Panamá.  Se lee el tipo de producto por si es Tradicional o Universal.
  ...
  -- Panamá. Se lee el tipo de cobertura para determimar si la cobertura pertenece a Bancario, Fidelidad, o Construccion
  ...
  -- Panamá. Se lee Si es Proveedor de personas
  ...
  -- Panamá. Se El digito de verificacion
  CURSOR lcuDigitoVerificacion IS
   SELECT NMDIGITO_VER
   FROM PERSONAS
   WHERE DNI       = lvaDni;

  -- Panamà.  Tipo de Coaseguro para detectar cuando es Aceptado
  ...
  --lobjTyinf.CDTOKEN          := ''; hasta donde sé, no aplica
...
----------------------------------------------------------------------------------------------------------------------
  ...
  FOR cab IN lcuDatosCabecera LOOP
   ...
   END IF;
   ---juancagr 20160903 se añade como parametro el valor de la factura y el numero de factura
   ...
   --Panamá.  Cuando para panamá se esté manejando GE, este parámetro va a venir en la tabla pagos
...
--SAP va a poner el dígito de verificación.
NULL;
--     lobjDocumento.nmBeneficiarioPago := lobjDocumento.nmBeneficiarioPago||':'||lvaDigitoVerificacion;
   ...
   IF lvaCdciaMatriz = '00' THEN
    IF doc.CDTRIBUTARIA = 'N' AND doc.CDRETENCION NOT IN ('0029','0028') THEN
     lobjDocumento.cdOperacion := lvaCdOperacion||'P';
    END IF;
   ELSIF lvaCdciaMatriz = '30' THEN
   -- Panamá En la coexistencia, no se pregunta por reembolso.  Solo se lee de personas si es proveedor
...
--    END IF;
   -- Panamá.  Riesgo del Exterior
     IF lvaCdRamo = '030' THEN
      ...
      IF lvaSnRiesgoExterior = 'S' THEN
       lobjDocumento.cdOperacion := lvaCdOperacion||'E';
      END IF;
     END IF;
   END IF;
    -- lobjDocumento.cdTipoReserva: Inicialmente tenía NULL por requerimiento de las pruevas, con el mensaje: Va en null porque no aplica el tipo de reserva para Fondo y Cesion
   ...
   -- Panamá. no se llevan las retenciones
   IF lvaCdciaMatriz = '00' THEN
    lvaCdRetencion                     := doc.CDRETENCION;
...
--Se comenta como estaba anteriomente y se llevan solo si hay código de retencion
...
--   IF NVL(lnuImpIrpfRetenido,0) <> 0 THEN
     ...
     --Se comenta esta línea por el cambio de calculo de retención desde SAP y avisado desde la devuelta.
     --para la base se envía el valor del IVA doc.IMPIRPF y no la base de toda la retención como se tenía acostumbrado.
     --    lobjRetencion.ptBaseRetencion       := doc.BASEIMPUESTOS;
    ...
    -- Retencion de IVA Régimen Simplificado
    ...
    IF lvaCdCompania IN ('01') THEN
     ...
     IF NVL(lnuImpIrpfRetenido,0) = 0 AND lvaSnIva = 'N' AND doc.CDTRIBUTARIA = 'N'
      ...
      --En el cambio del cálculo de retenciones, el reteIVA regimen simplificado se calcula y se envía a SAP calculado
...
--   IF NVL(lnuImpIca,0) <> 0 THEN
  ...
  --Llenado de la tabla de Detalles para tipo de reserva de Fondo de ahorro y Valores de Cesion.
   ...
   FOR reg IN lcuDatosPagosFyC LOOP
    ...
    -- linaduto - 2025/03/11 - HU756348 -Ajuste retención en la fuente para ID NIT - subramos PCP y deudores 
    -- luishure - 2025/12/10 - HU984785 - Exclusión de retención para Fondo de Ahorro   
    IF (lvaCdRamo = '081' AND lvaCdGarantia = 'VID') AND lvaAplicaReservaSinRetencion = 'N' THEN    
    ...
    -- Panamá 20130722 Compañía = 30 En Colombia es cdciaMatriz en SAP es 3000
    ...
    -- Panamá 20130722 el ramo es el que viene de la fuente
    IF lvaCdciaMatriz = '00' THEN
...
--     lobjDetalle.cdRamo  := 'MVI';
    ...
    ELSIF lvaCdciaMatriz = '30' THEN
     IF lvaCdRamo = '081' THEN
      ...
      -- 081TA Vida individual tradicional 1 año
      -- 081TR Vida individual tradicional Renovacion
      -- 081UA Vida individual Universal 1 año
     ...
     ELSIF lvaCdRamo = '012' THEN
      --Para identificar el tipo de cobertura Bancaria, Fidelidad, Construcción ...
     ...
     ELSE
      ...
      -- con titulo Seguimiento BI Panamá
...
--      lobjDetalle.cdRamoParametro := lvaCdRamo;
    ...
    --Se comenta por solicitud de catálogo de servicio 85529 Solicitada por Mario Franco y afirmada por SAP
...
/*
-- Se elimina este cursor en reunión del 20 de Noviembre con German Duque, Freddy Marin.
-- Se le consultó a Luis Gmo. Freyre quien acepta el cambio.
   FOR nodesc IN lcuDetalleSumadosIMPIRPF LOOP
    IF nodesc.IVANODESCONTABLE <> 0 THEN
     lvaCdRamo                       := nodesc.CDRAMO;
     lvaCdSubramo                    := nodesc.CDSUBRAMO;
     lvaCdGarantia                   := nodesc.CDGARANTIA;
     lvaCdSubGarantia                := nodesc.CDSUBGARANTIA;
...
--      lobjDetalle.cdRamo              := 'MVI';
     ...
     lobjDetalle.nmAsegurado         := lvaNmAsegurado;
     lobjDetalle.ptImporte           := nodesc.IVANODESCONTABLE;
     ...
     PCK_GESTION_PRODUCTOS.SPGET_COMPANIA_PRODUCTO( nodesc.cdramo,
                                                    nodesc.cdsubramo,
                                                    lvaCdCompania,
                                                    lvaCdciaMatriz,
...
--      lobjDetalle.cdRamo              := 'MVI';
     ...
     PCK_GESTION_PRODUCTOS.SPGET_COMPANIA_PRODUCTO( reg.cdramo,reg.cdsubramo,lvaCdCompania,lvaCdciaMatriz,lvaMensajeTecnicoExt,lvaMensajeUsuarioExt);
     -- Se comenta esta línea por cambio de Panamá.
--     lobjDetalle.cdCompaniaParametro := lvaCdCompania;
    ...
    -- Panamá 20130722 Compañía = 30 En Colombia es cdciaMatriz en SAP es 3000
     ...
     IF lvaCdciaMatriz = '00' THEN
      ...
      END IF;
     ELSIF lvaCdciaMatriz = '30' THEN
      --Para panamá el indicador de impuesto para el deducible es cero.
...
--     lobjDocumento.nmBeneficiarioPago := lobjDocumento.nmBeneficiarioPago||':'||lvaDigitoVerificacion;
   ...
   -- Pagos de Juridico de Pretenciòn Objetivada y de gastos de proceso Jurìdico se van por caja.
   --20140728 Por historia de usuario 77550 solicitada por Mario Franco, ya no se lleva obligado por Caja la pretención objetivada.
   /*
   lvaSNPagoJuridico := 'N';
   OPEN lcuSNPagoJuridico;
   FETCH lcuSNPagoJuridico INTO lvaSNPagoJuridico;
   IF lcuSNPagoJuridico%FOUND THEN
    lvaCdViaPago     := 'CA';
    -- Datos de la cuenta
    lobjCuentaBancaria.cdPaisBanco     := NULL;
    lobjCuentaBancaria.cdBanco         := NULL;
    lobjCuentaBancaria.nmCuenta        := NULL;
    lobjCuentaBancaria.cdTipoCuenta    := NULL;
    lobjCuentaBancaria.dsTitular       := NULL;
   END IF;
   CLOSE lcuSNPagoJuridico;
   */ -- Fin de 20140728 Por historia de usuario 77550...
...
--    END IF;
   ...
   -- Nos define como actuar.  parámetro 1 = 01; parámetro 2 = 041, parámetro 3 = T, parámetro 4 = 907 o 916 y parámetro 5 = 23.
   -- El parámetro 5 es el concepto que es lo unico que cambia.
  ...
  --Llenado de la tabla de Detalles para tipo de reserva Matematica y Técnina.
   lnuContadorDetalle := 0;
   lobjTabDetalles := tab_sap_detalles_cxp_sini(obj_sap_detalle_cxp_sini());
...
-- Se modifica el if por solicitud de catalogo de servicio 77046 20/05/2013 10:27:38 por Luis Gmo. Freyre
    ...
    END IF;
    IF  lvaDni = 'A8110368755' OR reg.NUMERO_RESERVA IN ('P012099','P012097') THEN
...
-- FIN DE COMENTAR BAMCASEGUROS
  ...
  -- Panamá.  Se incluye en cursor CDCIA_MATRIZ
  -- Banco de Bogotá, se incluye en el cursor CDRAMO, CDSUBRAMO
...
--   lvaMensajeTecnico := 'Consultando los datos de DNI y CDCIA';
   ...
   -- Panamá.  Se incluye en fetch lvaCdCompaniaMatriz
   -- Banco de Bogotá.  Se incluye en el fetch lvaCdRamo, lvaCdSubramo
...
--   lobjMensaje.CDCOMPANIA           := lvaCdCompania;
   ...
   --Banco de Bogota.  Se cambia el motivo de anulación que venía quemado en 04 a determinarlo de acuerdo al tipo de negocio Seguros o Bancaseguros.
--   lobjMensaje.CDMOTIVO_ANULACION   := '04';
   ...
   -- Panamá.  La compañía es la Matriz
...
----------------------------------------------------------------------------------------------------------------------------------------
  ...
  -- No se está usando en el momento porque se van a tomar sumados del documento
  /*
...
-- Cursores de Documentos
  ...
  --Llenado de la tabla de Detalles para tipo de reserva de Fondo de ahorro y Valores de Cesion.
   ...
   -----------------------------------------------------------------------------------------
    ...
    -- No se va a utilizar, dado que siempre se va a llenar la información del tercero.
...
----------------------------------------------------------------------------------------------------------------------
  ...
  FOR doc IN lcuDatosSumados LOOP
   ...
   IF NVL(doc.DEDUCIBLE,0) <> 0 THEN
    -- Para DEDUCIBLE participación del asegurado en el reclamo
    lnuContadorDocumentos            := lnuContadorDocumentos + 1;
...
--   lvaDsTextoPosicion                      := NULL;--'AUTORIZACIONES SALVAMENTOS';
   ...
   IF NVL(cab.CDRETENCION,0) <> '0099' THEN
    lvaCdConcepto := 'G';
    --Panamá.  en email de German Duque de miércoles 2013/11/06 11:31 a.m.
...
/*   OPEN lcuTipoIva;
   ...
   IF NVL(lvaCdRetencion,0) NOT IN ('0099','0000')  THEN
    ...
    --Se deja por si al futuro hay retencion de ICA y habría que tener la oficina para que retorne valor
    OPEN lcuMunicipioIca;
    FETCH lcuMunicipioIca INTO lvaMunicipioIca;
    CLOSE lcuMunicipioIca;
    lnuContadorRetencion                   := lnuContadorRetencion + 1;
...
----------------------------------------------------------------------------------------------------------------------
  --Datos de Creación de Deudor (beneficiario de pago del salvamento)
  OPEN lcuCdUsuario;
  FETCH lcuCdUsuario INTO lvaCdUsuario;
  CLOSE lcuCdUsuario;
...
-------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 --Variables para el manejo de Errores
  ...
  -- Panamá.  Se incluye en el cursor CDCIA_MATRIZ
  ...
  BEGIN
   ...
   -- Panamá.  Se incluye en el fetch lvaCdCiaMatriz
   OPEN lcuDatosPago;
...
-------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  BEGIN
   ...
   IF ovaDsUsuario IS NULL OR ovaDsClave IS NULL THEN
    ovaMensajeUsuario :='No existen parámetros para el usuario';
    ovaMensajeTecnico :='No existen parámetros para el usuario';
   END IF;
  END SP_USUARIO;
-------------------------------------------------------------------------------------------------------------------------------------------------
  FUNCTION FN_RIESGO_EXTERIOR (ivaNmPoliza         IN VARCHAR2,
...
-- Se elimina esta validación por solicitud de Juan Diego Cadavid en correo del : lunes, 29 de septiembre de 2014 03:59 p.m.
-- con Asunto: Quitar validación para deducibles Panamá
...
/*     lvaMensajeUsuario := 'No se han generado pagos al siniestro, no se pueden ingresar movimientos';
    ...
    -- Para Subrogación el campo ivaDniSubrogacion es obligatorio
    ...
    -- Verifica El código de Retenciones
    ...
    IF lnuNumeroPagoAutorizacion IS NOT NULL THEN
     ...
     IF lvaMensajeTecnicoExt IS NOT NULL OR  lvaMensajeUsuarioExt IS NOT NULL THEN
      ...
      RAISE lexErrorProcedimientoExt;
     END IF;
      -- INFORMACIÓN PARA SINESTROS DE COMPAÑÍAS EXTERNAS A SURA.
      -- Se tomo este codigo de cuando se tenía en Caja Integrada.
      DECLARE
      ...
      BEGIN
       ...
       END IF;
      END;
      -- FIN INFORMACIÓN PARA SINESTROS DE COMPAÑÍAS EXTERNAS A SURA
...
---------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    IF lvaNmPoliza <> ivaNmPoliza THEN
     lvaMensajeUsuario := 'La póliza enviada no corresponde a la poliza siniestrada.';
...
\*      OR ivaCdCodigoRetencion    IS NULL
  ...
  -- está UNDER el objeto de SuraBroker OBJ_SBK_MENSAJE
...
--  lnuTotalPtDescuento              SIN_PAGOS_DET.PTDESCUENTO%TYPE;
  ...
  CURSOR lcuDatosDocumentoFyC IS
   ...
   AND   s.CDSUBGARANTIA          = dac.CDSUBGARANTIA
   AND   NUMERO_RESERVA          IN ('P012091','P012081','P012093','P012096')
   GROUP BY s.CDGARANTIA, CDTIPO_RESERVA,NUMERO_RESERVA, NVL(POIRPF,0), S.CDRETENCION,NVL(POIRC,0), NVL(POIRPF_RETENIDO,0), NVL(POICA,0), /*ROUND(ABS(((IMPORTE*100)/(100+NVL(POIRPF,0))))),*/--Se cambia porque está generando mas de un documento cuando no hay cambio de retenciones
            s.CDTRIBUTARIA/*, s.SNIVA*/,
            EXPEDIENTE;
...
--   IF NVL(lnuImpIca,0) <> 0 THEN
  ...
  --Llenado de la tabla de Detalles para tipo de reserva de Fondo de ahorro y Valores de Cesion.
   ...
   FOR reg IN lcuDatosPagosFyC(doc.CDGARANTIA) LOOP
    ...
    -- 01/12/2024 josebuvi VERIFICACION DE VIDA PARA INDICADOR IMPUESTO
    --linaduto - 2025/03/11 - HU756348 -Ajuste retención en la fuente para ID NIT - subramos PCP y deudores
    IF (lvaCdRamo = '083' AND lvaCdGarantia = 'VID') THEN 
      IF (lvaAplicaSinRetencion = 'N' OR (lvaAplicaSinRetencion = 'S' AND substr(lvaDni,1,1) <> 'A')) THEN
          lnuContadorRetencionVida             := lnuContadorRetencionVida + 1;
          IF lnuContadorRetencionVida = 1 THEN
            lnuContadorRetencion               := lnuContadorRetencion + 1;
