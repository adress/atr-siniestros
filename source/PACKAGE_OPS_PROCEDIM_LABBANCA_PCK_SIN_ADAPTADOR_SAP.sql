...
--  lnuTotalPtDescuento              SIN_PAGOS_DET.PTDESCUENTO%TYPE;
  ...
  --fernavi - Identificar si el producto se contabiliza a una cuenta de personas en compañia 1000
  -- historia Cambios contabilización Productos Affinity- CXP
 ...
 CURSOR lcuDatosCaja(pIsDevolucion VARCHAR2, pUsaApiGee VARCHAR2) IS
  ...
  CURSOR lcuIsDevolucion IS
   SELECT 'S'
	  ...
	  AND p.cdramo = 'BAN'
	  AND p.cdsubramo = 'VMR'
	  AND p.cdgarantia = 'SBV'
      AND p.cdsubgarantia = 'NDX';
...
----------------------------------------------------------------------------------------------------------------------
  ...
  FOR cab IN lcuDatosCabecera LOOP
   ...
   OPEN lcuIsDevolucion;
   FETCH lcuIsDevolucion INTO lvIsDevolucion;
   IF lcuIsDevolucion%NOTFOUND THEN
	lvIsDevolucion := 'N';
   END IF;
...
--     lobjDetalle.cdRamo  := 'MVI';
    ...
    IF lvaCdciaMatriz = '00' THEN
     ...
     -- fernsavi -  si el producto contabiliza a una cuenta diferente se envia ramo contable
