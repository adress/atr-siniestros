    ...
    ELSE
      ...
      IF NVL(lvaUsaApiGeeSiniCxP, 'N') = 'S' THEN
        ...
        IF INSTR(UPPER(lvaRamosSinCxpSalud),UPPER(lvaCdRamo)) > 0 THEN
          ...
          PCK_CPI_INTEGRATION_V2.SP_ENVIAR_DOCUMENTO_ASYNC(lobjCaus, OBJ_CPI_JSON_CAUSACION_STRATEGY(1), 'TGLO_ASYNC_TX_1', 'SEQ_GLO_ASYNC_TX_NMID_1'); 
        ELSE
          lobjCaus := PCK_SIN_ADAPTADOR_CPI.MAP_SAP_CXP_TO_CAUSACION(
              lobjPago,
          ...
          PCK_CPI_INTEGRATION_V2.SP_ENVIAR_DOCUMENTO_ASYNC(lobjCaus, OBJ_CPI_JSON_CAUSACION_STRATEGY(1), 'TATR_ASYNC_TX_1', 'SEQ_ATR_ASYNC_TX_NMID_1');
        END IF;
      ELSE
        --------------------------------------------------------
...
BEGIN
  ...
  IF v_cnuevo = 'S' THEN
    BEGIN
      ...
      BEGIN
        ...
        ELSE
          ...
          -- === Decisi?n de fallback t?cnico/estructural por prefijos del paquete ===
          IF v_a_msj_tec IS NOT NULL THEN
            IF    UPPER(v_a_msj_tec) LIKE 'HEADER:%'
              ...
              lbEjecutarLegado := TRUE;   -- fallo t?cnico/estructural => fallback
            ELSE
              lbEjecutarLegado := FALSE;  -- mensaje de negocio => nos quedamos
            END IF;
          ELSE
            lbEjecutarLegado := FALSE;    -- OK t?cnico
          END IF;
        END IF;
      END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- Excepci?n t?cnica (timeout, HTTP no 2xx, etc.) => fallback legado
