CREATE OR REPLACE TYPE OPS$PROCEDIM.OBJ_CPI_CAUSACION_CONTABLE UNDER OPS$PROCEDIM.OBJ_CPI_DOCUMENTO (
...
CREATE OR REPLACE TYPE OPS$PROCEDIM.OBJ_CPI_JSON_CAUSACION_STRATEGY
...
/
CREATE OR REPLACE TYPE BODY OPS$PROCEDIM.OBJ_CPI_JSON_CAUSACION_STRATEGY AS
      ...
      IF p_obj.posiciones(v_idx).detalle.COUNT > 0 THEN
        FOR v_idx_param IN 1..p_obj.posiciones(v_idx).detalle.COUNT LOOP
          ...
          BEGIN
            ...
            v_detalle_datos_gen_obj.put('indicadorImpuestos', NVL(v_detalle_item.datosGenerales.indicadorImpuesto, ''));
